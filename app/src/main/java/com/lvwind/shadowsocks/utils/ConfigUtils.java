...
public class ConfigUtils {
    ...
    static public String PDNSD_LOCAL =
            "global { \n" +
                    "perm_cache = 2048;\n" +
                    "cache_dir = \"/data/data/com.idotools.vpn\";" +
                    "server_ip = %s;" +
                    "server_port = %d;" +
    ...
    public static boolean isLollipopOrAbove() {
        ...
        *
        SsConfig getPublicConfig(Context context,Container container,SsConfig config){
        ...
        new SsConfig(config.isGlobalProxy, config.isGFWList, config.isBypassApps, config.isTrafficStat,
        config.isUdpDns, config.profileName, host, password, method, config.proxiedAppString, config.route, port,
        config.localPort)
        }

        def load(settings: SharedPreferences): SsConfig = {
        ...
        val password = settings.getString(Key.password, "default")
        ...
        new SsConfig(isGlobalProxy, isGFWList, isBypassApps, isTrafficStat, isUdpDns, profileName, proxy,
        password, encMethod, proxiedAppString, route, remotePort, localPort)
        }
